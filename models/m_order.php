...
$requete = "SELECT P.id as prodId, odd.delivery_type, P.name, P.price, P.image, O.quantity, odd.total, D.* FROM products P JOIN orderitems O ON (P.id = O.product_id) JOIN orders odd ON (O.order_id = odd.id) JOIN delivery_addresses D ON (odd.delivery_add_id = D.id)  WHERE O.order_id = ?";
$donnees = array(
	$orderId
);
